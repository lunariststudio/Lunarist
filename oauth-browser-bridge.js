...
let running=false,booting=null,client=null;
async function getClient(){
  if(window.supabaseClient?.auth){client=window.supabaseClient;return client}
  if(client?.auth)return client;
  if(!window.supabase?.createClient)return null;
  try{
    const r=await fetch('/api/config',{cache:'no-store'});if(!r.ok)return null;
    const c=await r.json();if(!c?.url||!c?.anonKey)return null;
    client=window.supabase.createClient(c.url,c.anonKey,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true,flowType:'pkce'}});
    return client;
  }catch{return null}
}
async function session(){const sb=await getClient();try{return(await sb?.auth?.getSession?.())?.data?.session||null}catch{return null}}
...
async function continueOAuth(){
  ...
  if(!s?.access_token){try{window.toast?.('Please sign in to Lunarist to continue the connection.')}catch{}return}
...
function boot(){
  ...
  booting=(async()=>{
    if(new URLSearchParams(location.search).get('oauth_start')!=='1')return;
    for(let i=0;i<20;i++){await continueOAuth();if(document.getElementById('lunaristOAuthConsent')||running)return;await new Promise(r=>setTimeout(r,250));}
  })();
  const attach=async()=>{const sb=await getClient();try{sb?.auth?.onAuthStateChange?.((event)=>{if(event==='SIGNED_IN'||event==='TOKEN_REFRESHED')setTimeout(continueOAuth,50)})}catch{}};attach();
}
