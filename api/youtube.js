export default async function handler(req, res) {
  ...
  if (platform === 'x' || platform === 'twitter') {
    ...
    try {
      ...
      const metrics = post.public_metrics || {}, firstMedia=media[0]||null;
  ...
  if (platform === 'instagram' || platform === 'ig') {
    const input=String(req.query?.url||'').trim();
    try { const u=new URL(input),host=u.hostname.toLowerCase().replace(/^www\./,''); if(host!=='instagram.com'&&!host.endsWith('.instagram.com'))throw Error(); } catch { return res.status(400).json({error:'Invalid Instagram URL'}); }
    ...
    try {
      const endpoint=new URL('https://graph.facebook.com/v22.0/instagram_oembed');endpoint.searchParams.set('url',input);endpoint.searchParams.set('access_token',token);endpoint.searchParams.set('omitscript','true');
      const r=await fetch(endpoint),data=await r.json();if(!r.ok)return res.status(r.status).json({error:data?.error?.message||'Instagram API request failed'});
      return res.status(200).json({platform:'instagram',url:input,title:data.title||'',authorName:data.author_name||'',authorUrl:data.author_url||'',thumbnailUrl:data.thumbnail_url||'',html:data.html||'',provider:data.provider_name||'Instagram',likes:null,views:null,mediaUrl:input,mediaType:'image'});
