...
// X media is rendered as a clean, edge-to-edge player; profile/embed chrome is removed.
(function(){
  ...
  function style(){
    ...
    s.textContent=`
      ...
      body.lunarist-x-video-mode .modalbox{position:relative!important;width:100vw!important;height:100dvh!important;max-width:none!important;max-height:none!important;margin:0!important;border:0!important;border-radius:0!important;background:#000!important;display:block!important;overflow:hidden!important}
      body.lunarist-x-video-mode .modalmedia{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;aspect-ratio:auto!important;background:#000!important;display:flex!important;align-items:center!important;justify-content:center!important}
      body.lunarist-x-video-mode .modalmedia iframe{display:none!important}
      body.lunarist-x-video-mode .modalmedia video{display:block!important;width:100%!important;height:100%!important;max-width:none!important;max-height:none!important;object-fit:contain!important;border-radius:0!important;background:#000!important}
      body.lunarist-x-video-mode .modalcontent{position:absolute!important;left:0!important;right:0!important;bottom:0!important;z-index:4!important;padding:70px 24px 24px!important;background:linear-gradient(transparent,rgba(0,0,0,.78) 38%,rgba(0,0,0,.96))!important;pointer-events:none!important}
      ...
      body.lunarist-x-video-mode .modalclose{z-index:12!important;right:14px!important;top:14px!important}
      .lunarist-x-toolbar{position:absolute;top:14px;left:14px;right:64px;z-index:11;display:flex;align-items:center;justify-content:flex-start;gap:10px;pointer-events:none}
      ...
      @media(max-width:720px){body.lunarist-x-video-mode .modalcontent{padding:58px 14px 16px!important}.lunarist-x-toolbar{top:9px;left:9px;right:58px}.lunarist-x-metric{padding:7px 9px;font-size:11px}}
    `;
  ...
  function currentXId(){
    ...
    if(!modal) return '';
    const candidates=[
      modal.getAttribute('data-x-url'),
      modal.getAttribute('data-media-url'),
      modal.querySelector('[data-x-url]')?.getAttribute('data-x-url'),
      modal.querySelector('[data-media-url]')?.getAttribute('data-media-url')
    ];
    for(const value of candidates){const id=parseId(value||'');if(id)return id;}
    for(const a of modal.querySelectorAll('a[href]')){const id=parseId(a.href);if(id)return id;}
    for(const f of modal.querySelectorAll('iframe[src]')){const id=parseId(f.src);if(id)return id;}
  ...
  function currentProjectId(){
    ...
    const values=[
      ...
      modal.querySelector('[data-project-id]')?.getAttribute('data-project-id'),
      modal.querySelector('[data-id]')?.getAttribute('data-id')
    ];
    for(const v of values) if(uuid.test(String(v||''))) return String(v);
    return '';
  }
  ...
  async function syncProjectMetrics(projectId,d){
    ...
    const likes=Number.isFinite(Number(d.likes))?Number(d.likes):null;
    const views=Number.isFinite(Number(d.views))?Number(d.views):null;
    if(likes===null&&views===null)return;
    try{
      await fetch('/api/x',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({projectId,xId:String(d.id||''),likes,views})});
    }catch{}
  }
  ...
  function renderMetrics(modal,d){
    ...
    if(!host){
      ...
      host=document.createElement('div');host.className='lunarist-x-metrics';content.appendChild(host);
    }
  ...
  function installVideo(modal,d){
    ...
    if(video.src!==d.mediaUrl){video.src=d.mediaUrl;video.load();}
  ...
  observer.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['class','src','data-project-id','data-x-url']});
