...
(function(){
  ...
  async function loadCardUrl(){
    ...
    try{const r=await supabaseClient.from('profiles').select('eugene_card_url').eq('id',state.currentUser.id).maybeSingle();cardUrl=r.data?.eugene_card_url||'';}catch(e){}
    return cardUrl;
  }
  ...
  function addNavButton(){
    ...
    const b=document.createElement('button');b.className='navbtn';b.id='navEugeneCardBtn';b.textContent=state.language==='ja'?'Eugene Card':'Eugene Card';b.title='Connect Eugene Card';b.onclick=async()=>{await loadCardUrl();openModal()};
