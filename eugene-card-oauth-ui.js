(function(){
  ...
  const STATUS='/api/eugene-oauth-status';
  ...
  function statusNodes(){
    return[...document.querySelectorAll('body *')].filter(el=>{
      ...
      return t==='Checking connection...'||t==='Connected'||t==='Not connected';
    });
  }
  ...
  document.addEventListener('click',e=>{
    ...
    if(t==='connect eugene card'||t==='connect account'||t.includes('connect your eugene card')){
      e.preventDefault();e.stopImmediatePropagation();startOAuth();
    }
    if(t==='disconnect eugene card'||t==='disconnect account'||t.includes('disconnect eugene')){
      e.preventDefault();e.stopImmediatePropagation();disconnect();
    }
  },true);
