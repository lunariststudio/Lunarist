...
function config(){
  // OAuth admin endpoints must use a server-only privileged key. Never use a NEXT_PUBLIC/anon key here.
  const url=(process.env.SUPABASE_URL||process.env.NEXT_PUBLIC_SUPABASE_URL||'').replace(/\/$/,'');
  const key=(process.env.SUPABASE_SERVICE_ROLE_KEY||process.env.SUPABASE_SERVICE_KEY||process.env.SUPABASE_SECRET_KEY||'').trim();
  return{url,key};
}
...
export async function handleOAuthAdmin(req,res){
  ...
  if(!url)return res.status(503).json({error:'SUPABASE_URL is not configured on the server.'});
  if(!key)return res.status(503).json({error:'SUPABASE_SERVICE_ROLE_KEY is not configured on the server. Add it to the Vercel Production environment and redeploy.'});
  if(!(await admin(url,key,req)))return res.status(403).json({error:'Administrator access required.'});
  const action=String(req.query?.admin_action||req.query?.action||req.body?.action||'list-clients');
