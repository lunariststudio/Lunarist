...
const CALLBACK=`${ORIGIN}/api/eugene-card/callback`;
...
// Eugene Card is the OAuth client. Lunarist only issues the authorization code,
// exposes the OAuth provider endpoints, and bridges the registered callback back
// to Eugene so Eugene can redeem the code with its original PKCE verifier.
export async function callback(req,res){
  ...
  // Do not redeem here. Eugene must send the original code_verifier to /api/lunarist/exchange.
  return redirect(res,{connect:'lunarist',code,state});
}

export async function start(req,res){
  return res.status(200).json({provider:'Lunarist',client_id:CLIENT_ID,authorization_endpoint:`${ORIGIN}/oauth/authorize`,token_endpoint:`${ORIGIN}/oauth/token`,userinfo_endpoint:`${ORIGIN}/oauth/userinfo`,revocation_endpoint:`${ORIGIN}/oauth/revoke`,redirect_uri:CALLBACK,scope:'openid profile email offline_access',pkce:'S256',start_url:EUGENE_RETURN});
}
