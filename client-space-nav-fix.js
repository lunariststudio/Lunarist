...
(function(){
  ...
  const isPublicProfile=()=>{const p=location.pathname.replace(/^\/+|\/+$/g,'');return !!p&&!p.includes('/')&&!/^(discover|artists|services|commissions|admin|api)$/i.test(p);};
  document.addEventListener('click',(event)=>{const button=event.target?.closest?.('#navlinks .navbtn');if(!button||button.id==='navClientSpaceBtn')return;const current=typeof window.routeFromPath==='function'?window.routeFromPath(location.pathname):null;if(current!=='clients'&&!isPublicProfile())return;const route=routeFromButton(button);if(!route||typeof window.pathForRoute!=='function')return;const path=window.pathForRoute(route);if(!path)return;event.preventDefault();event.stopImmediatePropagation();window.goRoute?.(route);},true);
